...
docker compose up -d
cp .env.example .env
go run cmd/lodester/main.go
```

### 方式 B: 直接インストール（Proxmox / Docker 非対応環境）

```bash
# PostgreSQL にユーザーと DB を作成
psql -U postgres -c "CREATE USER lodester WITH PASSWORD 'dev_password_change_me';"
psql -U postgres -c "CREATE DATABASE lodester_dev OWNER lodester;"

# マイグレーション実行
goose -dir migrations postgres "postgresql://lodester:dev_password_change_me@localhost:5432/lodester_dev?sslmode=disable" up

# サーバー起動
DATABASE_URL="postgresql://lodester:dev_password_change_me@localhost:5432/lodester_dev?sslmode=disable" go run cmd/lodester/main.go
```
...
```bash
# ヘルスチェック
curl http://localhost:8080/.well-known/oap/health

# アカウント作成
curl -X POST http://localhost:8080/api/v1/accounts \
  -H "Content-Type: application/json" \
  -d '{"email":"test@example.com","login_hash":"dummy_hash","kdf_params":{"algorithm":"argon2id","memory":65536,"iterations":3,"parallelism":4}}'
